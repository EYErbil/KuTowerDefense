...
public class Knight extends Enemy implements Serializable {
    ...
    public Knight(double x, double y) {
        super(x, y, KNIGHT_WIDTH, KNIGHT_HEIGHT, KNIGHT_HEALTH, KNIGHT_BASE_SPEED, KNIGHT_GOLD_REWARD, EnemyType.KNIGHT);
    ...
    public boolean update(double deltaTime, List<Enemy> allEnemies) {
        ...
        // GameMap.TILE_SIZE might be better sourced from GameMap instance if available, or a global constant
        // For now, assuming a known tile width like 64.0
        double tileWidthThreshold = GameMap.TILE_SIZE; // Or a hardcoded 64.0 if GameMap.TILE_SIZE is not static/accessible
        ...
        // The actual movement and status effect application (like slow) is done in super.update
        // super.update will use the this.speed we just set, and then apply slowFactor if isSlowed is true.
